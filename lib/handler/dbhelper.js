...
class dbhelper {
    ...
    getNextValue = async (rangeName) => {
        ...
        let nextValueResult;
        try {
            nextValueResult = await SELECT.one.columns('CurrentValue','IncrementBy').from(Ranges).where({ RangeName: rangeName }).forUpdate({ wait: 5 });
        } catch (err) {
            throw new Error(`Failed to acquire lock for range ${rangeName}: ${err.message}`);
        }
